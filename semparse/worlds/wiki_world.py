...
class WikiWorld:
    ...
    def __init__(self, db_context: WikiDBContext, nl_context: WikiNLContext , query: Optional[List[str]], allow_alias: bool = False) -> None:
        ...
        def remove_from_clause(query: Optional[List[str]]) -> Optional[List[str]]:
            """
            Pre-processing the query to remove the "FROM" clauses.
            """
            if query is None:
                return query
            ...
            while i < len(query):
                ...
                if not flag and tok == '(':
                    flag1 = True
                    inc += 1
                if not flag and flag1 and tok == ')':
                    inc -= 1
                    flag1 = False
                if tok == 'WHERE' or tok == 'GROUP' or tok == 'ORDER' or tok == 'where' or (
                        tok == ')' and inc == 0) or tok == 'group' or tok == 'order' or tok == 'FROM_COUNT' or tok == 'from_count':
                    flag = True
                if flag == True:
                    toks.append(tok)
                i += 1
