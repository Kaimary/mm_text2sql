...
from dataset_readers.dataset_utils import  read_wiki_dataset_schema,TableColumn, read_wiki_dataset_values
...
class WikiDBContext:
    ...
    def entity_key_for_column(table_name: str,column: TableColumn)-> str:
        ...
        #todo wiki 的 column.name 是 col0...n 这里不确定是.name 还是 .text
    ...
    def get_db_knowledge_graph(self,db_id: str) -> KnowledgeGraph:
        ...
        if db_id not in self.db_tables_data:
            self.db_tables_data[db_id] = read_wiki_dataset_values(db_id, self.tables_file, tables)
        ...
        for table in tables:
            ...
            entity_text[table_key] = table.text         # table.name = table.text =db_id
        ...
        for table_name in db_schema.keys():
            for column in db_schema[table_name].columns:
                ...
                other_column = [col for col in db_schema[other_column_table].columns if col.name == other_column_name][
                    0]
    ...
    def normalize_string(string: str) -> str:
        #todo 这个我完全照搬spider 这个是不是可以用来normalize wiki中的cell value 我没有验证过
        """
    ...
    def get_entities_from_question(self,
        ...
        for entity in self._expand_entities(self.tokenized_utterance, entity_data, string_column_mapping):
            if entity["token_type"] == "text":
                # 只有text类型的最终被返回了  str : list 的tuple
