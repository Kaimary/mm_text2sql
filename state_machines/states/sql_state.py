...
class SqlState:
        ...
        if current_clause in ['select_core']:
            for rule_id, rule in zip(valid_actions_ids, valid_actions_rules):
                ...
                if lhs == 'column_name' and self._is_in_where_subq_clause():
                    ...
                    #columns_used_by_where_stack: [tablename@columname]

                    last_column_in_where = self.columns_used_by_where_stack[-1:]
                    last_column_related_table = last_column_in_where.split('@')[0]
                    last_column_name = last_column_in_where.split('@')[1]
                    if candidate_column == last_column_name and candidate_related_table == last_column_related_table:
                        continue

                    last_column_foreign_key = ""
                    for key in self.schema.keys():
                        if key.lower() == last_column_related_table:
                            for column in self.schema[key].columns:
                                if column.name.lower() == last_column_name:
                                    last_column_foreign_key = column.foreign_key
                                    break
                    if last_column_foreign_key:
    ...
    def _is_in_where_subq_clause(self):
        ...
        is_select_core_flag = False
        is_subq_flag = False

        for rule in self.current_stack[::-1]:
            if rule[0] in relevant_clauses:
                if rule[0] == 'select_core' and not is_select_core_flag and  not is_subq_flag:
                    is_select_core_flag = True
                    continue
                elif rule[0] == 'source_subq' and is_select_core_flag and  not is_subq_flag:
                    is_subq_flag = True
                    continue
                elif rule[0] == 'where_clause' and is_select_core_flag and is_subq_flag:
                    return True
                else:
                    return False
